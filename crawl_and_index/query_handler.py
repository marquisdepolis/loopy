...
def get_answer_using_chatbot(text_chunk, query, api_key):
    ...
    message = [{
        "role": "system",
        "content": "You are a very helpful McKinsey consultant"
    }]
    message.append({"role": "user", "content": text_chunk})
...
def answer_query(query, api_key, min_similarity=0.5, top_n_chunks=3):
    ...
    sorted_chunks = sorted(filtered_chunks, key=lambda x: x[0], reverse=True)[:top_n_chunks]
